...
use crate::hints::{
    bootloader_hints::{
        assert_is_composite_packed_output, assert_program_address, compute_and_configure_fact_topologies, enter_packed_output_scope,
        guess_pre_image_of_subtasks_output_hash, import_packed_output_schemas, is_plain_packed_output, load_bootloader_config,
        prepare_simple_bootloader_input, prepare_simple_bootloader_output_segment, restore_bootloader_output, save_output_pointer,
        save_packed_outputs, set_packed_output_to_subtasks,
    },
    codes::*,
    execute_task_hints::{
        allocate_program_data_segment, append_fact_topologies, call_task, exit_scope_with_comments, load_program_hint, validate_hash,
        write_return_builtins_hint,
    },
    inner_select_builtins::select_builtin,
    select_builtins::select_builtins_enter_scope,
    simple_bootloader_hints::{
        divide_num_by_2, prepare_task_range_checks, set_ap_to_zero, set_ap_to_zero_or_one, set_current_task, set_tasks_variable,
    },
};
...
impl HintProcessorLogic for MinimalBootloaderHintProcessor {
    fn execute_hint_extensive(
        ...
        let hint_extension = match hint_data.code.as_str() {
            ...
            BOOTLOADER_PREPARE_SIMPLE_BOOTLOADER_INPUT => prepare_simple_bootloader_input(exec_scopes),
            BOOTLOADER_LOAD_BOOTLOADER_CONFIG => load_bootloader_config(vm, exec_scopes, ids_data, ap_tracking),
            BOOTLOADER_ENTER_PACKED_OUTPUT_SCOPE => enter_packed_output_scope(vm, exec_scopes, ids_data, ap_tracking),
            BOOTLOADER_SAVE_OUTPUT_POINTER => save_output_pointer(vm, exec_scopes, ids_data, ap_tracking),
            ...
            BOOTLOADER_COMPUTE_FACT_TOPOLOGIES => compute_and_configure_fact_topologies(vm, exec_scopes),
            ...
            BOOTLOADER_ASSERT_IS_COMPOSITE_PACKED_OUTPUT => assert_is_composite_packed_output(exec_scopes),
            SIMPLE_BOOTLOADER_PREPARE_TASK_RANGE_CHECKS => prepare_task_range_checks(vm, exec_scopes, ids_data, ap_tracking),
            ...
            SIMPLE_BOOTLOADER_ZERO_V0_13_1 => set_ap_to_zero_or_one(vm, exec_scopes, ids_data, ap_tracking),
            EXECUTE_TASK_ALLOCATE_PROGRAM_DATA_SEGMENT => allocate_program_data_segment(vm, exec_scopes, ids_data, ap_tracking),
            EXECUTE_TASK_LOAD_PROGRAM => load_program_hint(vm, exec_scopes, ids_data, ap_tracking),
            EXECUTE_TASK_VALIDATE_HASH_V0_13_0 => validate_hash(vm, exec_scopes, ids_data, ap_tracking),
            EXECUTE_TASK_VALIDATE_HASH_V0_13_1 => validate_hash(vm, exec_scopes, ids_data, ap_tracking),
            EXECUTE_TASK_ASSERT_PROGRAM_ADDRESS => assert_program_address(vm, exec_scopes, ids_data, ap_tracking),
            ...
            EXECUTE_TASK_APPEND_FACT_TOPOLOGIES => append_fact_topologies(vm, exec_scopes, ids_data, ap_tracking),
            ...
            INNER_SELECT_BUILTINS_SELECT_BUILTIN => select_builtin(vm, exec_scopes, ids_data, ap_tracking),
            unknown_hint_code => Err(HintError::UnknownHint(unknown_hint_code.to_string().into_boxed_str())),
        };
